...
AnalysisResult Run(std::istream& input, std::ostream& output, const Config& config)
{
    ...
    try {
        const auto shapes = ShapeParser::ParseAll(input);
        ...
        if (config.enableRender && !shapes.empty()) {
...
void RunRender(const std::vector<std::unique_ptr<IShape>>& shapes, uint32_t windowWidth, uint32_t windowHeight)
{
    try {
